...
export class VentasService {
  ...
  async crearVenta(createVentaDto: CreateVentaDto): Promise<Venta> {
    ...
    // 1. Validación de seguridad elemental
    ...
    // 3. Recorrer el arreglo de productos e insertarlos en la tabla detalles_ventas
    for (const prod of productos) {
      ...
      const precioBase = Number(productoDB.precio_base);
      ...
      totalAcumuladoVenta += detalleInsertado.precio_subtotal;
      await this.ventaRepository.save(ventaGuardada)

      return ventaGuardada
    }

    // Retornamos la venta guardada con su ID autogenerado
    return ventaGuardada;
  }
